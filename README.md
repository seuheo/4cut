...
## 📝 변경 이력 (v13.1)
- 테스트 모드 이미지 경로 수정: 로컬 경로 → 실제 URL(Picsum)로 변경하여 미리보기 정상 표시
- FrameApplyViewModel DI/Null-safety 보강: Navigation에서 Factory로 PhotoRepository/FrameRepository/Context 주입
- 미리보기/저장 안정화: 프레임 선택 즉시 미리보기 생성 및 갤러리 저장 성공
- README 상태 블록 정리: 중복된 하단 상태 섹션을 최신 상태와 일치하도록 수정
