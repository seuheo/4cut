...
### 🎯 Phase 4: 새로운 홈 화면 및 데이터 영구 저장 (0% 시작)

**목표**: 기존 홈 화면을 사용자의 사진 기록을 보여주는 '포토로그' 형태로 전면 개편하고, 이를 위해 앱 내에 데이터를 영구적으로 저장할 수 있는 기반을 마련합니다.

#### Phase 4.1: 데이터 저장 기반 구축 (Room 데이터베이스)
- [ ] `build.gradle.kts` 파일에 Room 라이브러리 의존성 추가
- [ ] `data` 패키지 내에 `database` 폴더 생성
- [ ] `PhotoEntity.kt` 생성: 저장할 사진 정보(ID, 이미지 경로, 생성 날짜, 위치 등)를 담는 테이블 정의
- [ ] `PhotoDao.kt` 생성: 데이터베이스에 사진 정보를 추가하고 조회하는 함수(DAO) 인터페이스 정의
- [ ] `AppDatabase.kt` 생성: Room 데이터베이스 클래스 정의
- [ ] `FrameViewModel.kt` 수정: 이미지 저장 시, 갤러리 저장과 더불어 Room 데이터베이스에도 사진 정보를 저장하는 로직 추가

#### Phase 4.2: 새로운 홈 화면 UI 구현
- [ ] `HomeViewModel.kt` 수정: Room 데이터베이스에 저장된 사진 목록과 총 개수를 불러오는 기능 구현
- [ ] `HomeScreen.kt` UI 전면 개편:
    - 기존 KTX 일러스트, 프레임 캐러셀, 캘린더 UI 제거
    - "KTX와 함께한 N개의 추억" 타이틀 추가
    - 저장된 사진 목록을 보여주는 가로 스크롤 캐러셀 UI 구현
    - "새로운 네컷사진 만들기" 버튼 구현
- [ ] (신규) `components` 폴더에 `PhotoLogCard.kt` 컴포저블 생성: 홈 화면 캐러셀에 들어갈 개별 사진 카드 UI 구현
...
- [ ] 간단한 스티커 및 텍스트 추가 기능
- [ ] GIF(움짤) 촬영 모드
- [ ] **(신규) Phase 4: 새로운 홈 화면 및 포토로그 기능 구현**
...
**마지막 업데이트**: 2024년 12월 19일  
**현재 상태**: Phase 3.3.4 완료 (100%), MVP v1.0 최종 완성 ✅, Phase 4 준비 중  
